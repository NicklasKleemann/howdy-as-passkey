...
// verify runs the Howdy PAM stack via pamtester and returns true ONLY on a
// clean success. Every other outcome — non-zero exit, missing binary, timeout,
// any error — returns false. Fail closed: we never assert verification we did
...
func (c *HowdyClient) verify(reason string) bool {
	...
	cmd := exec.CommandContext(ctx, "pamtester", c.pamService, c.user, "authenticate")
